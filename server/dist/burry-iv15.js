...
exports.getFullIV15Analysis = getFullIV15Analysis;
// ═══════════════════════════════════════════════════════════════════
...
const IV15_CACHE = new Map();
const IV15_CACHE_TTL = 4 * 60 * 60 * 1000; // 4 hours
async function gfFetch(endpoint) {
    try {
...
function computeTragicAlgebra(gf, price) {
...
function computePVWithDilution(gf, tragicAlgebra) {
...
function computeAdjustedROIC(gf) {
...
function classifyAICT(gf, symbol) {
    ...
    if (isSoftware) {
        score += 15;
        factors.push('Software/SaaS sector — inherent switching cost advantage');
    }
    else if (isFintech) {
        score += 10;
        factors.push('Fintech — moderate switching costs');
    }
    else if (isHardware) {
        score += 5;
        factors.push('Hardware — some IP moat but commodity risk');
    }
    else if (isHealthcare) {
        score += 5;
        factors.push('Healthcare — regulatory moat possible');
    }
    else if (isServices) {
        score -= 5;
        factors.push('Services — limited structural moat');
    }
    else if (isTraditional) {
        score -= 10;
        factors.push('Traditional industry — weaker competitive moats');
    }
    // Gross margin as moat proxy
    if (gf.grossMargin !== null) {
        if (gf.grossMargin > 80) {
            score += 20;
            factors.push(`Gross margin ${gf.grossMargin.toFixed(0)}% — elite pricing power`);
        }
        else if (gf.grossMargin > 65) {
            score += 10;
            factors.push(`Gross margin ${gf.grossMargin.toFixed(0)}% — strong pricing power`);
        }
        else if (gf.grossMargin > 50) {
            score += 0;
            factors.push(`Gross margin ${gf.grossMargin.toFixed(0)}% — moderate`);
        }
        else if (gf.grossMargin > 30) {
            score -= 10;
            factors.push(`Gross margin ${gf.grossMargin.toFixed(0)}% — limited pricing power`);
        }
        else {
            score -= 15;
            factors.push(`Gross margin ${gf.grossMargin.toFixed(0)}% — commodity-like`);
        }
    }
    ...
    if (gf.roic !== null) {
        if (gf.roic > 25) {
            score += 15;
            factors.push(`ROIC ${gf.roic.toFixed(0)}% — exceptional capital allocation`);
        }
        else if (gf.roic > 15) {
            score += 10;
            factors.push(`ROIC ${gf.roic.toFixed(0)}% — above cost of capital`);
        }
        else if (gf.roic > 8) {
            score += 0;
        }
            ...
            score -= 10;
            factors.push(`ROIC ${gf.roic.toFixed(0)}% — below cost of capital`);
        }
        else {
            score -= 15;
            factors.push(`Negative ROIC — destroying value`);
        }
    }
    ...
    if (gf.mktcap !== null) {
        ...
        if (mktcapB > 500) {
            score += 10;
            factors.push('Mega-cap — network effects and scale moat');
        }
        else if (mktcapB > 50) {
            score += 5;
            factors.push('Large-cap — established market position');
        }
        else if (mktcapB < 2) {
            score -= 5;
            factors.push('Small-cap — scale disadvantage');
        }
    }
    ...
    if (gf.fcfMargin !== null) {
        if (gf.fcfMargin > 30) {
            score += 5;
            factors.push(`FCF margin ${gf.fcfMargin.toFixed(0)}% — cash machine`);
        }
        else if (gf.fcfMargin < 0) {
            score -= 10;
            factors.push('Negative FCF — cash burn');
        }
    }
    // Map score to tier
    let tier;
...
function computeIV15(gf, aict, adjustedROIC, tragicAlgebra, price) {
...
function computeCompositeScore(gf, tragicAlgebra, iv15, adjustedROIC, price) {
    // ── Shareholder's Bucket ──
    let shScore = 50;
    const shComponents = {};
    // Dilution impact
    if (tragicAlgebra.dilutionRate > 0.02) {
        shScore -= 20;
    }
    else if (tragicAlgebra.dilutionRate > 0.01) {
        shScore -= 10;
    }
    else if (tragicAlgebra.dilutionRate === 0 && gf.sharesBuybackRate !== null && gf.sharesBuybackRate > 2) {
        shScore += 20;
    }
    else if (gf.sharesBuybackRate !== null && gf.sharesBuybackRate > 0) {
        shScore += 10;
    }
    shComponents.dilutionImpact = Math.round(tragicAlgebra.dilutionRate * 10000) / 100;
    // SBC burden
    if (tragicAlgebra.gaapOverstatement !== null) {
        if (tragicAlgebra.gaapOverstatement > 30) {
            shScore -= 15;
        }
        else if (tragicAlgebra.gaapOverstatement > 15) {
            shScore -= 5;
        }
        else if (tragicAlgebra.gaapOverstatement < 5) {
            shScore += 10;
        }
    ...
    if (gf.buybackYield !== null) {
        if (gf.buybackYield > 6) {
            shScore += 15;
        }
        else if (gf.buybackYield > 3) {
            shScore += 5;
        }
    ...
    if (gf.opMargin !== null) {
        ...
        else if (gf.opMargin > 20) {
            qScore += 5;
        }
        else if (gf.opMargin < 5) {
            qScore -= 10;
        }
    ...
    if (gf.piotroskiF !== null) {
        ...
        else if (gf.piotroskiF >= 5) {
            qScore += 5;
        }
        else if (gf.piotroskiF <= 3) {
            qScore -= 10;
        }
    ...
    if (gf.debtToEbitda !== null) {
        if (gf.debtToEbitda < 1) {
            qScore += 5;
        }
        else if (gf.debtToEbitda > 4) {
            qScore -= 15;
        }
    ...
    if (iv15.priceToIV15 !== null) {
        if (iv15.priceToIV15 < 0.7) {
            vScore += 30;
        }
        else if (iv15.priceToIV15 < 0.9) {
            vScore += 20;
        }
        else if (iv15.priceToIV15 < 1.1) {
            vScore += 5;
        }
        else if (iv15.priceToIV15 > 2.0) {
            vScore -= 25;
        }
        else if (iv15.priceToIV15 > 1.5) {
            vScore -= 15;
        }
        else if (iv15.priceToIV15 > 1.2) {
            vScore -= 5;
        }
    ...
    if (gf.fcfYield !== null) {
        ...
        else if (gf.fcfYield > 5) {
            vScore += 5;
        }
        else if (gf.fcfYield < 1) {
            vScore -= 10;
        }
    ...
    if (gf.gfValueMargin !== null) {
        if (gf.gfValueMargin < -30) {
            vScore += 10;
        }
        else if (gf.gfValueMargin > 30) {
            vScore -= 10;
        }
    ...
    if (verdict === 'ELITE')
        parts.push('Burry would have this high on his watchlist.');
    else if (verdict === 'POOR')
        parts.push('Multiple red flags across all dimensions — Burry would avoid entirely.');
    return {
        overall,
        verdict,
        buckets: {
...
function classifyAllMapZone(iv15, aict, composite) {
    ...
    else if (priceToIV15 <= fatThreshold * 1.15 && compositeRank >= compositeMin * 0.85) {
        zone = 'JUST_OUTSIDE';
...
// MASTER FUNCTION
// ═══════════════════════════════════════════════════════════════════
async function getFullIV15Analysis(symbol, price) {
    ...
    if (!gf) {
        const empty = {
            ...
            composite: { overall: 0, verdict: 'POOR', buckets: { shareholders: { score: 0, weight: 0.35, components: {} }, quality: { score: 0, weight: 0.35, components: {} }, valuation: { score: 0, weight: 0.30, components: {} } }, narrative: 'Data unavailable.' },
    ...
    // Data gaps
    const dataGaps = [];
    if (gf.sbcPerShare === null)
        dataGaps.push('SBC per share not available — dilution impact approximated from share count');
    if (aict.confidence !== 'HIGH')
        dataGaps.push(`AICT tier is heuristic (${aict.confidence} confidence) — not from Burry's published SW50`);
    ...
    if (gf.revGrowth3yr === null)
        dataGaps.push('3-year revenue growth unavailable — using YoY growth as proxy');
    const result = {
        symbol: sym,
        price,
        dataAvailable,
...
//# sourceMappingURL=burry-iv15.js.map