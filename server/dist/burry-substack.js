...
router.get('/burry/llm-test', async (_req, res) => {
    ...
    try {
        const anthropic = new sdk_1.default({ apiKey, timeout: 15000 });
        const msg = await anthropic.messages.create({
            model: 'claude-sonnet-4-6',
            max_tokens: 50,
...
async function evaluateBurryFramework(input) {
    ...
    if (fwdPe !== null) {
        if (fwdPe < 12) {
            ...
            valScore -= 25;
            valNotes.push(`Forward P/E ${fwdPe.toFixed(0)}x — pricing in perfection. Burry: "I don't pay for hope"`);
        }
        else if (fwdPe > 35) {
            valScore -= 15;
            ...
            valScore -= 10;
            valNotes.push('Forward P/E > 25 — above Burry comfort zone');
        }
    }
    ...
    if (fcfYield !== null) {
        if (fcfYield > 8) {
            ...
            valScore -= 15;
            valNotes.push('FCF yield < 1% — near-disqualifier for a value investor');
        }
    }
    ...
    if (upDownRatio !== null) {
        if (upDownRatio > 1.5) {
            ...
            volScore = Math.max(0, volScore - 20);
            volDetail += '. Distribution volume dominates — smart money exiting';
        }
    }
    ...
    if (rsi14 !== null) {
        if (rsi14 < 25) {
            contrScore += 20;
            contrNotes.push(`RSI ${rsi14.toFixed(0)} — deeply oversold, peak contrarian buy signal`);
        }
        else if (rsi14 < 30) {
            contrScore += 15;
            ...
            contrScore -= 25;
            contrNotes.push(`RSI ${rsi14.toFixed(0)} — extreme euphoria, crowd mania territory`);
        }
        else if (rsi14 > 70) {
            contrScore -= 15;
            contrNotes.push(`RSI ${rsi14.toFixed(0)} — overbought, herd is fully loaded`);
        }
    }
