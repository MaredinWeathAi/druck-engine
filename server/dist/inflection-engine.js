...
function computeExtendedTA(ticker, name, bars, spyCloses) {
    ...
    let daysSinceCross200d = null;
    let extensionVelocity = null;
    if (sma200.length > 1) {
...
function classifyPhaseStructural(ta, currentPhase) {
    ...
    if (ta.volume) {
        ...
        if (structuralPhase === 'BUYING_EXHAUSTION') {
            if (ta.volume.volumeExhaustion === 'buying') {
                confidence += 10;
                transitions.push('Buying volume exhaustion — climax top risk');
            }
            if (ta.volume.climaxVolume) {
                confidence += 5;
                transitions.push('Volume climax detected — distribution');
            }
        }
    }
    // RSI confirmation
    if (ta.rsi14 !== null) {
        if (structuralPhase === 'NARRATIVE_EXPANSION' && ta.rsi14 >= 50 && ta.rsi14 <= 70)
            confidence += 10;
        if (structuralPhase === 'NARRATIVE_EXPANSION' && ta.rsi14 > 70) {
            confidence -= 5;
            transitions.push(`RSI ${ta.rsi14.toFixed(0)} — momentum elevated`);
        }
        if (structuralPhase === 'BUYING_EXHAUSTION' && rsiOverbought) {
            confidence += 10;
            transitions.push(`RSI ${ta.rsi14.toFixed(0)} — overbought confirms exhaustion`);
        }
        if (structuralPhase === 'BUYING_EXHAUSTION' && rsiOverextended) {
            confidence += 5;
            transitions.push(`RSI ${ta.rsi14.toFixed(0)} — extreme overbought`);
        }
        if (structuralPhase === 'SELLING_EXHAUSTION' && rsiOversold) {
            confidence += 15;
            transitions.push(`RSI ${ta.rsi14.toFixed(0)} — deeply oversold inflection zone`);
        }
        if (structuralPhase === 'NARRATIVE_REVERSAL' && rsiDeepOversold) {
            confidence += 10;
            transitions.push(`RSI ${ta.rsi14.toFixed(0)} — extreme oversold, inflection likely`);
        }
    }
    // 52-week position
    if (ta.highLow) {
        if (structuralPhase === 'NARRATIVE_EXPANSION' && ta.highLow.pctFromHigh > -5)
            confidence += 5;
        if (structuralPhase === 'NARRATIVE_EXPANSION' && ta.highLow.pctFromHigh < -15) {
            confidence -= 10;
            transitions.push('Well off highs — trend quality questionable');
        }
        if (structuralPhase === 'NARRATIVE_REVERSAL' && ta.highLow.pctFromHigh < -25)
            confidence += 5;
        if (structuralPhase === 'SELLING_EXHAUSTION' && ta.highLow.pctFromHigh < -30) {
            confidence += 10;
    ...
    if (ta.failedBreaks.length > 0) {
        ...
        if (recentFailed.some(b => b.type === 'failed_breakout')) {
            ...
            if (structuralPhase === 'BUYING_EXHAUSTION') {
                confidence += 10;
                transitions.push('Failed breakout confirms exhaustion');
            }
        }
        if (recentFailed.some(b => b.type === 'failed_breakdown')) {
            if (structuralPhase === 'SELLING_EXHAUSTION') {
                confidence += 10;
