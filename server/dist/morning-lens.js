...
function isLLMDisabled() {
    if (llmConsecutiveFailures < LLM_MAX_FAILURES)
        return false;
...
// ─── ARIA NARRATIVE ───
async function generateAriaNarrative() {
    if (!currentSignals)
...
End with a portfolio implication sentence for Maredin's book. Use only the attached JSON state — do not invent data not in it. If a field is null or stale, say so.`,
        ...
        if (!text)
            return 'Narrative unavailable: no API key configured or LLM returned empty response.';
...
async function fetchTickerBars(symbol, years = 2) {
    ...
    let staleCacheBackup = null;
    try {
        const ageMin = (0, history_store_1.getBarCacheAge)(symbol);
        if (ageMin !== null) {
            const dbCached = (0, history_store_1.getCachedBars)(symbol);
            if (dbCached && dbCached.bars.length >= 50) {
                const hasVolume = dbCached.bars.some((b) => (b.volume || 0) > 0);
    ...
    const staleData = staleCacheBackup || (() => { try {
        return (0, history_store_1.getCachedBars)(symbol)?.bars || null;
    }
    catch {
        return null;
    } })();
...
function generateRuleBasedNarrative(d) {
    ...
    if (rs)
        p2 += `Relative strength ${rs}. `;
    if (failedBDs > 0)
        p2 += `${failedBDs} failed breakdown(s) detected — historically bullish pattern. `;
...
function generateInvestmentCases(d, verdict, phaseInfo) {
    ...
    if (earnings.length >= 2) {
        ...
        if (improving) {
            bullEvidence.push(`Earnings trajectory improving (${lastTwo.map((e) => (e > 0 ? '+' : '') + e.toFixed(1) + '%').join(' → ')}) — market is rewarding results at an accelerating rate.`);
        }
    ...
    // Bull catalysts
    if (!goldenCross)
        bullCatalysts.push('Reclaim of 200d MA and formation of golden cross would confirm trend reversal');
    if (ext200 !== null && ext200 < 0)
        bullCatalysts.push('Price reclaim above 200d MA would shift institutional sentiment');
    if (failedBDs === 0)
        bullCatalysts.push('Failed breakdown at current support would trap shorts and build a launchpad');
    bullCatalysts.push('Positive earnings surprise or guidance raise triggering narrative shift');
    if (udv !== null && udv < 1.0)
        bullCatalysts.push('Volume rotation from distribution to accumulation (Up/Down > 1.3)');
    if (rsi !== null && rsi < 30)
        bullCatalysts.push('RSI reversal from oversold — mean reversion bounce');
    ...
    if (earnings.length >= 2) {
        ...
        if (deteriorating) {
            bearEvidence.push(`Earnings trajectory deteriorating (${lastTwo.map((e) => (e > 0 ? '+' : '') + e.toFixed(1) + '%').join(' → ')}) — each report is received worse than the last.`);
        }
    ...
    // Bear catalysts
    if (goldenCross)
        bearCatalysts.push('Death cross formation (50d crossing below 200d) would confirm structural trend change');
    if (ext200 !== null && ext200 > 0)
        bearCatalysts.push('Loss of 200d MA support would trigger institutional stop-loss cascades');
    bearCatalysts.push('Negative earnings surprise or guidance cut confirming fundamental deterioration');
    if (udv !== null && udv > 1.0)
        bearCatalysts.push('Volume shift from accumulation to distribution (Up/Down < 0.7)');
    if (failedBDs > 0)
        bearCatalysts.push('Clean breakdown through previously held support would eliminate the floor');
    bearCatalysts.push('Sector headwind, macro deterioration, or credit tightening');
    if (rsi !== null && rsi > 65)
        bearCatalysts.push('RSI rollover from overbought — momentum exhaustion');
    // Bear thesis
...
async function warmAllCaches() {
    ...
    for (let i = 0; i < allSymbols.length; i++) {
        ...
        try {
            const ageMin = (0, history_store_1.getBarCacheAge)(sym);
            if (ageMin !== null && ageMin < 10080) { // <7 days
                const dbCached = (0, history_store_1.getCachedBars)(sym);
                if (dbCached && dbCached.bars.length >= 50) {
                    const hasVolume = dbCached.bars.some((b) => (b.volume || 0) > 0);
                    if (hasVolume) {
                        skipped++;
        ...
        try {
            ...
            if (quotes.length >= 50) {
                ...
                (0, history_store_1.setCachedBars)(sym, bars);
...
router.get('/lens/cache-status', (_req, res) => {
    try {
