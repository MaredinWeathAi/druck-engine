...
class AnomalyDetector {
    calculateSurpriseMagnitude(signalType, actual, expected) {
        if (expected === 0)
            expected = 0.0001;
    ...
    assessCascadePotential(signalType, impact, affectedSectors, marketCapPct) {
        if (signalType === 'litigation' && (impact === 'major' || impact === 'critical')) {
            if (marketCapPct > 2)
                return 'high';
        }
        if (signalType === 'macro_event')
            return 'high';
        if (signalType === 'earnings_surprise' && marketCapPct > 1)
            return 'medium';
        if (signalType === 'supply_chain' && affectedSectors.length > 2)
            return 'high';
        return 'low';
    }
    calculateActionRecommendation(signalType, direction, impact, cascade, portfolioRelevance) {
        if (impact === 'critical') {
        ...
        if (impact === 'significant')
            return { action: 'monitor_closely', urgency: false };
        if (impact === 'monitor')
            return { action: 'monitor', urgency: false };
    ...
    detectEarningsSurprise(params) {
        ...
        const direction = epsBeatPct > 0 ? 'positive' : 'negative';
        let impact;
        ...
        if (absBeat < 5)
            ...
            impact = 'major';
        if (guidance && Math.abs(stockReactionPct) > 10)
            impact = 'significant';
    ...
    detectMacroEvent(params) {
        ...
        const direction = impactEstimate > 0 ? 'positive' : impactEstimate < 0 ? 'negative' : 'mixed';
    ...
    detectSentimentShift(params) {
        ...
        if (spikeMultiplier >= 10)
            impact = 'major';
        else if (spikeMultiplier >= 5)
            impact = 'significant';
        else if (spikeMultiplier >= 2)
            impact = 'monitor';
        else
            impact = 'noise';
        const direction = sentimentScore > 20 ? 'positive' : sentimentScore < -20 ? 'negative' : 'mixed';
    ...
    detectGuruMove(params) {
        ...
        let impact;
        if (positionPct > 20)
            impact = 'major'; // Huge conviction (like Burry 66% PLTR)
        else if (positionPct > 10)
            impact = 'significant';
        else if (changePct > 50)
            impact = 'significant'; // Large add
        else
            impact = 'monitor';
...
class SignalPublisher {
    ...
    search(query = '', filters = {}) {
        ...
        if (query) {
            ...
            results = results.filter(h => h.headline.toLowerCase().includes(q) || h.ticker.toLowerCase().includes(q));
        }
        if (filters.ticker)
            results = results.filter(h => h.ticker === filters.ticker);
        if (filters.impact)
            results = results.filter(h => h.impactLevel === filters.impact);
        if (filters.signalType)
            results = results.filter(h => h.signalType === filters.signalType);
        if (filters.direction)
            results = results.filter(h => h.direction === filters.direction);
        if (filters.urgency !== undefined)
            results = results.filter(h => h.urgency === filters.urgency);
        if (filters.source)
...
class DigestGenerator {
    ...
    generateDigest(runTime) {
        const headlines = this.publisher.getRecent(500);
        const now = new Date();
        const critical = this.buildSection('CRITICAL ALERTS', headlines.filter(h => h.impactLevel === 'critical'), 'critical');
        const major = this.buildSection('MAJOR EVENTS', headlines.filter(h => h.impactLevel === 'major'), 'major');
        const sectors = this.buildSection('SECTOR ROTATIONS', headlines.filter(h => h.signalType === 'sector_rotation'), 'sector');
        const macro = this.buildSection('MACRO IMPACTS', headlines.filter(h => h.signalType === 'macro_event'), 'macro');
        const portfolio = this.buildSection('YOUR PORTFOLIO', headlines.filter(h => h.portfolioRelevance === 'your_position' || h.portfolioRelevance === 'guru_holding'), 'portfolio');
    ...
    buildSection(title, headlines, type) {
        ...
        if (neg > 0)
            takeaway += `${neg} negative`;
        if (pos > 0)
            takeaway += `${neg > 0 ? ', ' : ''}${pos} positive`;
        takeaway += '.';
        return {
            title,
    ...
    buildExecutiveSummary(runTime, headlines, critical, major) {
        const lines = [];
...
// ============================================================================
function loadGuruPositions() {
    // Try loading from server/data directory
    const csvPaths = [
        path_1.default.join(__dirname, '..', 'data', 'guru_top30_positions_q1_2026.csv'),
        path_1.default.join(__dirname, '..', '..', 'data', 'guru_top30_positions_q1_2026.csv'),
    ];
...
// ============================================================================
function generateDemoSignals(detector, guruPositions) {
    const signals = [];
    const now = new Date();
...
router.get('/intel/status', (req, res) => {
...
router.get('/intel/headlines', (req, res) => {
...
router.get('/intel/search', (req, res) => {
    ...
    if (req.query.impact)
        filters.impact = req.query.impact;
    if (req.query.signal_type)
        filters.signalType = req.query.signal_type;
    if (req.query.direction)
        filters.direction = req.query.direction;
    if (req.query.urgency === 'true')
        filters.urgency = true;
    if (req.query.source)
...
router.get('/intel/headlines/:ticker', (req, res) => {
...
router.get('/intel/critical', (req, res) => {
...
router.get('/intel/digest', (req, res) => {
    const runTime = req.query.run_time === '3:30pm' ? '3:30pm' : '11:30am';
    const digest = digestGenerator.generateDigest(runTime);
    res.json(digest);
});
// GET /intel/research — Research queue
router.get('/intel/research', (req, res) => {
    ...
    const queue = [];
    for (const h of criticals)
        queue.push({ priority: 'P5-CRITICAL', ticker: h.ticker, headline: h.headline, action: h.actionRecommended, deepDive: h.deepDiveLink, deadline: 'ASAP (1-2 hours)' });
    for (const h of majors)
        queue.push({ priority: 'P4-MAJOR', ticker: h.ticker, headline: h.headline, action: h.actionRecommended, deepDive: h.deepDiveLink, deadline: 'Today' });
    for (const h of portfolio)
        queue.push({ priority: 'P3-PORTFOLIO', ticker: h.ticker, headline: h.headline, action: h.actionRecommended, deepDive: h.deepDiveLink, deadline: 'Tomorrow' });
    res.json({ queue, count: queue.length });
});
// GET /intel/guru — Guru positions
router.get('/intel/guru', (req, res) => {
    res.json({ positions: guruPositions, count: guruPositions.length });
});
// GET /intel/guru/:ticker — Guru positions for specific ticker
router.get('/intel/guru/:ticker', (req, res) => {
    const ticker = req.params.ticker.toUpperCase();
...
router.post('/intel/signal', (req, res) => {
    try {
        const body = req.body;
        let signal;
        switch (body.type) {
...
router.post('/intel/refresh', (req, res) => {
    demoLoaded = false;
    initializeSystem();
    const stats = publisher.getStats();
...
//# sourceMappingURL=market-intel.js.map