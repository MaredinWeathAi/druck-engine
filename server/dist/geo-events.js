...
exports.setGeoEventKeys = setGeoEventKeys;
...
// ============================================================================
async function analyzeEventsWithClaude(newsItems, anthropicApiKey) {
...
Only include events with genuine market impact. If nothing significant is happening, return {"events": []}. Return ONLY valid JSON.`
        ...
        return (parsed.events || []).map((evt, idx) => {
            const effectChains = (evt.effects || []).map((e) => ({
                ...e,
            ...
            for (const chain of effectChains) {
                for (const ticker of chain.tickers) {
                    if (!exposureMap.has(ticker) || chain.order < exposureMap.get(ticker).order) {
            ...
            return {
                ...
                region: evt.region,
                sources: (evt.sourceIndices || []).map((i) => newsItems[i]?.source || 'unknown'),
                effectChains,
 ...
 */
function classifyEventsLocally(newsItems) {
    const events = [];
    const keywords = {
    ...
    for (const item of newsItems) {
        ...
        for (const [keyword, config] of Object.entries(keywords)) {
            if (lower.includes(keyword)) {
                ...
                if (template) {
                    const effectChains = template.effects.map(e => ({
                        ...e,
                        tickers: SECTOR_TICKER_MAP[e.sector] || [],
                    }));
    ...
    for (const evt of events) {
        if (!byCategory.has(evt.category))
            byCategory.set(evt.category, []);
...
async function fetchEarningsCalendar() {
    ...
    try {
        const yahooFinance = await Promise.resolve().then(() => __importStar(require('yahoo-finance2')));
        ...
        for (let i = 0; i < TRACKED_TICKERS.length; i += batchSize) {
            ...
            entries.push(...results.filter((r) => r !== null));
...
async function fetchGuruFocusEarnings(apiKey) {
    if (!apiKey)
        return [];
    const entries = [];
    const now = new Date();
...
// MAIN SCAN FUNCTION
// ============================================================================
async function runEventScan(anthropicApiKey, guruFocusApiKey) {
    const now = Date.now();
    // Check cache
...
let _anthropicKey = '';
let _guruFocusKey = '';
function setGeoEventKeys(anthropicKey, guruFocusKey) {
 ...
 */
router.get('/exposure/:ticker', (req, res) => {
    const ticker = req.params.ticker.toUpperCase();
    const exposedEvents = eventStore.filter(evt => evt.portfolioExposure.some(p => p.ticker === ticker) ||
        evt.effectChains.some(c => c.tickers.includes(ticker))).map(evt => ({
        eventId: evt.id,
        title: evt.title,
        severity: evt.severity,
        category: evt.category,
 ...
 */
router.get('/effect-templates', (req, res) => {
...
Return ONLY valid JSON.`
        ...
        if (parsed.effects) {
            parsed.effects = parsed.effects.map((e) => ({
                ...e,
                tickers: SECTOR_TICKER_MAP[e.sector] || [],
            }));
        }
...
//# sourceMappingURL=geo-events.js.map