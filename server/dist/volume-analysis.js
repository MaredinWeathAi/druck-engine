...
exports.computeVolumeBreakdown = computeVolumeBreakdown;
exports.computeWatchlistVolumeBadge = computeWatchlistVolumeBadge;
const yahoo_finance2_1 = __importDefault(require("yahoo-finance2"));
// ── YAHOO FINANCE INITIALIZATION ──
let yahooFinance;
try {
    yahooFinance = new yahoo_finance2_1.default({ suppressNotices: ['yahooSurvey'] });
}
catch {
    try {
...
function computeCumulativeVolume(bars, peakIdx) {
    let cum = 0;
...
function detectPricePhase(bars, peakInfo) {
    ...
    if (makingLowerLows && recent30dReturn < -0.05)
        return 'ACTIVE_DECLINE';
    if (recent30dReturn > 0.10)
        return 'RECOVERING';
    if (rangeWidth < 0.12 && !makingLowerLows)
        return 'STABILIZING';
    if (recent30dReturn > 0.02 && !makingLowerLows)
        return 'RECOVERING';
    if (recent30dReturn < -0.03)
        return 'ACTIVE_DECLINE';
    return 'STABILIZING';
}
// ── VOLUME CHARACTER ANALYSIS ──
function analyzeVolumeCharacter(bars) {
    if (bars.length < 60)
        return null;
    const volBars = bars.filter(b => b.volume > 0);
    if (volBars.length < 60)
        return null;
    ...
    for (let i = volBars.length - lookback; i < volBars.length; i++) {
        ...
        if (volMultiple >= 2.0) {
            ...
            let type = 'EXPANSION';
            if (priceChange < -3 && volMultiple >= 2.5)
                type = 'CLIMAX_DOWN';
            else if (priceChange > 3 && volMultiple >= 2.5)
                type = 'CLIMAX_UP';
            else if (Math.abs(priceChange) < 1 && volMultiple >= 2.0)
                type = 'EXHAUSTION';
            spikes.push({
                date: bar.date,
                volume: bar.volume,
...
function findAbnormalVolumeEvents(bars, peakIdx) {
    if (bars.length < 60)
        return [];
    ...
    for (let i = Math.max(peakIdx, 50); i < bars.length; i++) {
        ...
        if (multiple >= 3.0) {
            ...
            if (priceChange < -5)
                ...
                classification = 'DISTRIBUTION_DAY';
            else if (priceChange > 5)
                classification = 'INSTITUTIONAL_BUY';
            else if (priceChange > 2)
                classification = 'ACCUMULATION_DAY';
            else if (Math.abs(priceChange) < 1)
                classification = 'CHURNING';
            events.push({
                date: bar.date,
                volume: bar.volume,
...
function interpretVolume(pricePhase, turnoverPct, volumeChar, abnormalEvents, declinePct) {
...
function checkVolumeConfirmedEntry(pricePhase, turnoverPct, volumeChar, declinePct) {
    if (!turnoverPct || !volumeChar) {
...
// ═══════════════════════════════════════════════════════════════════
async function computeVolumeBreakdown(symbol, bars) {
    ...
    let shareholderTurnover = null;
    if (sharesOutstanding && sharesOutstanding > 0) {
        const turnoverPct = (cumVolume / sharesOutstanding) * 100;
        let turnoverLabel = 'Early Rotation';
        if (turnoverPct >= 200)
            turnoverLabel = 'Rotation Complete';
        else if (turnoverPct >= 100)
            turnoverLabel = 'Progressing';
    ...
    const interpretation = interpretVolume(pricePhase, shareholderTurnover?.turnoverPct || null, volumeChar, abnormalEvents, peakInfo.declinePct);
    // 10. Volume-confirmed entry check
    const volumeConfirmedEntry = checkVolumeConfirmedEntry(pricePhase, shareholderTurnover?.turnoverPct || null, volumeChar, peakInfo.declinePct);
...
async function computeWatchlistVolumeBadge(symbol, bars) {
    const peakInfo = detectPeak(bars);
    if (!peakInfo || peakInfo.declinePct > -10) {
        return {
            symbol,
...
//# sourceMappingURL=volume-analysis.js.map