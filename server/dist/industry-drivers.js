...
// ============================================================================
function analyzeIndustry(industry, driverInputs) {
    ...
    let cyclicalPosition = 'mid_cycle';
    if (inflectionScore > 70 && activeSignals >= drivers.length * 0.5)
        cyclicalPosition = 'early_cycle';
    else if (inflectionScore > 55)
        cyclicalPosition = 'mid_cycle';
    else if (inflectionScore > 35)
        cyclicalPosition = 'late_cycle';
    else
        cyclicalPosition = 'downturn';
    // Recommendation
    let recommendation = '';
    if (cyclicalPosition === 'early_cycle')
        recommendation = `${config.label} showing early-cycle recovery signals — ${activeSignals}/${drivers.length} drivers improving. Consider overweight.`;
    else if (cyclicalPosition === 'mid_cycle')
        recommendation = `${config.label} in mid-cycle — mixed signals. Selective positioning in best-in-class names.`;
    else if (cyclicalPosition === 'late_cycle')
        recommendation = `${config.label} showing late-cycle dynamics — watch for deterioration. Reduce to quality names only.`;
    else
        recommendation = `${config.label} in downturn — most drivers deteriorating. Avoid or position for recovery.`;
    return {
        industry,
...
router.post('/:industry/analyze', (req, res) => {
    const industry = req.params.industry.toLowerCase();
...
router.post('/batch', (req, res) => {
    ...
    const results = analyses.map((a) => analyzeIndustry(a.industry, a.drivers || {}));
    res.json({ analyzed: results.length, results });
});
exports.default = router;
//# sourceMappingURL=industry-drivers.js.map