...
import {
  ...
  recordModelPerformance, getModelPerformanceHistory, setWatchlistEntryPrice,
} from './history-store';
...
let seedDiagnostics: { loaded: boolean; seedPath: string | null; seedSymbols: number; seeded: number; existingCached: number; pathsTried: string[]; errors: string[] } = {
...
async function fetchOHLC(symbol: string, period: string = '1y'): Promise<OHLCBar[]> {
  ...
  if (!yahooBlockDetected) try {
    ...
    const chartResp = await Promise.race([
      ...
      new Promise<never>((_, rej) => setTimeout(() => rej(new Error('raw_yahoo_timeout')), 8000)), // 8s (was 12s)
    ]);
    if (chartResp.ok) {
      const chartData: any = await chartResp.json();
...
async function refreshMorningLens(): Promise<void> {
  ...
  for (const inst of INSTRUMENTS) {
    const age = getBarCacheAge(inst.symbol);
    if (age !== null && age < 43200) cachedCount++;
  }
  ...
  if (failedSymbols.length > 0 && failedSymbols.length < INSTRUMENTS.length * 0.5 && !yahooBlockDetected) {
    ...
    for (const sym of failedSymbols) {
      if (yahooBlockDetected) break; // Stop retrying if block detected mid-retry
      try {
  ...
  if (previousSize > 0 && newSize < previousSize * MIN_SUCCESS_RATIO) {
    ...
    for (const [symbol, snap] of newSnapshots) {
      if (snap.phaseData) {
        ...
        try {
          const d = snap.daily || {} as any;
          const ext = (d.price && d.ma200) ? ((d.price / d.ma200 - 1) * 100) : 0;
          recordPhaseVerdictSnapshot(symbol, 'morning_lens', {
...
router.get('/lens/regime', (req: Request, res: Response) => {
  ...
  const bucketPhases: Record<string, Record<string, number>> = {};
  ...
  for (const phase of phaseNames) { phaseCounts[phase] = 0; }
  ...
  for (const snap of withPhase) {
    ...
    if (!bucketPhases[snap.bucket]) {
      ...
      for (const p of phaseNames) bucketPhases[snap.bucket][p] = 0;
    }
  ...
  const bucketSummaries: Record<string, any> = {};
...
function generateRuleBasedNarrative(d: any): string {
  ...
  const archetype = rawArchetype.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase());
...
async function detectSectorETFs(symbol: string): Promise<{ primary: string; secondary: string; sectorName: string }> {
  try {
    const url = `https://api.gurufocus.com/public/user/${GURUFOCUS_API_KEY}/stock/${symbol}/summary`;
    const response = await fetch(url);
    if (!response.ok) return { primary: 'SPY', secondary: 'RSP', sectorName: 'Broad Market' };
    const data: any = await response.json();
    ...
    // Match by keywords
    if (group.includes('homebuilder') || group.includes('residential construction')) return { primary: 'ITB', secondary: 'XHB', sectorName: 'Homebuilders' };
    if (group.includes('semiconductor') || group.includes('chip')) return { primary: 'SMH', secondary: 'XLK', sectorName: 'Semiconductors' };
    if (group.includes('software') || group.includes('saas')) return { primary: 'IGV', secondary: 'XLK', sectorName: 'Software' };
    if (group.includes('airline')) return { primary: 'JETS', secondary: 'IYT', sectorName: 'Airlines' };
    if (group.includes('bank') || group.includes('savings') || group.includes('lending')) return { primary: 'KRE', secondary: 'XLF', sectorName: 'Banks' };
    if (group.includes('biotech') || group.includes('pharma')) return { primary: 'XBI', secondary: 'XLV', sectorName: 'Biotech/Pharma' };
    if (group.includes('insurance')) return { primary: 'KIE', secondary: 'XLF', sectorName: 'Insurance' };
    if (group.includes('oil') || group.includes('gas') || group.includes('energy')) return { primary: 'XLE', secondary: 'XOP', sectorName: 'Energy' };
    if (group.includes('retail') || group.includes('store')) return { primary: 'XRT', secondary: 'XLY', sectorName: 'Retail' };
    if (group.includes('auto') || group.includes('vehicle')) return { primary: 'XLY', secondary: 'SPY', sectorName: 'Consumer Discretionary' };
    if (group.includes('medical') || group.includes('health') || group.includes('hospital')) return { primary: 'XLV', secondary: 'IHI', sectorName: 'Healthcare' };
    if (group.includes('transport') || group.includes('railroad') || group.includes('freight')) return { primary: 'IYT', secondary: 'XLI', sectorName: 'Transports' };
    if (group.includes('chemical') || group.includes('materials') || group.includes('metal') || group.includes('mining')) return { primary: 'XLB', secondary: 'SLX', sectorName: 'Materials' };
    if (group.includes('reit') || group.includes('real estate')) return { primary: 'VNQ', secondary: 'XLRE', sectorName: 'Real Estate' };
    if (group.includes('utility') || group.includes('electric') || group.includes('water')) return { primary: 'XLU', secondary: 'XLU', sectorName: 'Utilities' };
    if (group.includes('food') || group.includes('beverage') || group.includes('consumer staple')) return { primary: 'XLP', secondary: 'PBJ', sectorName: 'Consumer Staples' };
    if (group.includes('aerospace') || group.includes('defense')) return { primary: 'ITA', secondary: 'XLI', sectorName: 'Aerospace & Defense' };
    if (group.includes('media') || group.includes('telecom') || group.includes('communication')) return { primary: 'XLC', secondary: 'SPY', sectorName: 'Communications' };

    // Fall back to sector level
    if (sector.includes('technology') || sector.includes('tech')) return { primary: 'XLK', secondary: 'QQQ', sectorName: 'Technology' };
    if (sector.includes('financial')) return { primary: 'XLF', secondary: 'KRE', sectorName: 'Financials' };
    if (sector.includes('health')) return { primary: 'XLV', secondary: 'XBI', sectorName: 'Healthcare' };
    if (sector.includes('energy')) return { primary: 'XLE', secondary: 'XOP', sectorName: 'Energy' };
    if (sector.includes('industrial')) return { primary: 'XLI', secondary: 'IYT', sectorName: 'Industrials' };
    if (sector.includes('consumer') && sector.includes('disc')) return { primary: 'XLY', secondary: 'XRT', sectorName: 'Consumer Discretionary' };
    if (sector.includes('consumer') && sector.includes('stap')) return { primary: 'XLP', secondary: 'PBJ', sectorName: 'Consumer Staples' };
    if (sector.includes('material')) return { primary: 'XLB', secondary: 'SLX', sectorName: 'Materials' };
    if (sector.includes('communication')) return { primary: 'XLC', secondary: 'SPY', sectorName: 'Communications' };
...
router.post('/lens/watchlist/add', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (bars.length > 0 && spyBars.length > 0) {
      setWatchlistEntryPrice(symbol, bars[bars.length - 1].close, spyBars[spyBars.length - 1].close);
    }
  } catch {}
...
router.get('/lens/watchlist/phase-log', (req: Request, res: Response) => {
...
router.post('/lens/ai-analysis', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const phaseDistro: Record<string, number> = { P1: 0, P2: 0, P3: 0, P4: 0, P5: 0 };
    const sectorPhases: Record<string, { p1: string[]; p2: string[]; p5: string[] }> = {};
    const extremeExtensions: { symbol: string; name: string; ext: number; phase: string }[] = [];
    const transitions: { symbol: string; from: string; to: string; direction: string }[] = [];

    if (instrumentSnapshots) {
      for (const [sym, snap] of Object.entries(instrumentSnapshots)) {
        const s = snap as any;
        if (!s.phaseData) continue;
        const pn = s.phaseData.phaseNum;
        const pk = `P${pn}`;
        phaseDistro[pk] = (phaseDistro[pk] || 0) + 1;

        const group = s.group || 'Other';
        if (!sectorPhases[group]) sectorPhases[group] = { p1: [], p2: [], p5: [] };
        if (pn === 1) sectorPhases[group].p1.push(sym);
        else if (pn === 2) sectorPhases[group].p2.push(sym);
        else if (pn === 5) sectorPhases[group].p5.push(sym);
    ...
    const wlSummary = watchlist.map((w: any) => ({
      symbol: w.symbol,
      price: w.price,
      phase: w.phase_short,
      verdict: w.verdict,
    ...
    const phaseLog = getWatchlistPhaseLog();

    // 4. Historical accuracy data
    const historyRecords = getPhaseVerdictHistory(undefined, 500);
    const uniqueDates = new Set(historyRecords.map((h: any) => (h.recorded_at || '').slice(0, 10)));
...
FORMAT: Use clear section headers. Write concisely. No filler. No disclaimers.`,
    ...
    res.json({
      ...
      metadata: {
        ...
        phaseDistro,
        conflicts: wlSummary.filter((w: any) => w.sizingConflict).length,
      },
    });
  } catch (err: any) {
...
router.get('/lens/model-performance', async (req: Request, res: Response) => {
  try {
    // 1. Phase verdict history — compute forward returns where possible
    const allHistory = getPhaseVerdictHistory(undefined, 10000);
    const now = new Date();
    ...
    const symbolFirstRecord: Record<string, { price: number; phase: string; verdict: string; date: string; archetype: string }> = {};
    for (const h of allHistory) {
      if (!h.symbol || !h.price) continue;
    ...
    if (instrumentSnapshots) {
      for (const [sym, snap] of instrumentSnapshots.entries()) {
        const s = snap as any;
        if (s.daily?.price) currentPrices[sym] = s.daily.price;
      }
    }
    ...
    const phasePerf: Record<string, { returns: number[]; count: number }> = {};
    for (const [sym, first] of Object.entries(symbolFirstRecord)) {
      const curr = currentPrices[sym];
      if (!curr || !first.price || first.price <= 0) continue;
      const ret = (curr / first.price - 1) * 100;
      const phase = first.phase || 'Unknown';
      if (!phasePerf[phase]) phasePerf[phase] = { returns: [], count: 0 };
      phasePerf[phase].returns.push(ret);
      phasePerf[phase].count++;
    }

    // Compute averages
    const phasePerfSummary: Record<string, { avgReturn: number; count: number; vsSpyAlpha: number | null }> = {};
    ...
    const watchlist = getWatchlist();
    const wlPerformance = watchlist.map((w: any) => {
      ...
      let modelCorrect: string | null = null;
      if (tickerReturn !== null && w.verdict) {
        ...
        if (bullishVerdict && tickerReturn > 0) modelCorrect = 'CORRECT';
        else if (bullishVerdict && tickerReturn < -5) modelCorrect = 'WRONG';
        else if (bearishVerdict && tickerReturn < 0) modelCorrect = 'CORRECT';
        else if (bearishVerdict && tickerReturn > 5) modelCorrect = 'WRONG';
        else modelCorrect = 'INCONCLUSIVE';
      }
    ...
    const wlWithAlpha = wlPerformance.filter((w: any) => w.alpha !== null);
    const avgAlpha = wlWithAlpha.length > 0 ? wlWithAlpha.reduce((a: number, b: any) => a + b.alpha, 0) / wlWithAlpha.length : null;
    const correctCount = wlPerformance.filter((w: any) => w.modelCorrect === 'CORRECT').length;
    const wrongCount = wlPerformance.filter((w: any) => w.modelCorrect === 'WRONG').length;

    // 5. Historical performance log
    const perfHistory = getModelPerformanceHistory(undefined, 50);

    // 6. Sizing regime accuracy from history
    const sizingSignals: Record<string, { total: number; profitable: number }> = {
    ...
    for (const w of wlPerformance) {
      if (w.sizingRegime && w.tickerReturn !== null && sizingSignals[w.sizingRegime]) {
        ...
        if (w.sizingRegime === 'BACK_UP_TRUCK' || w.sizingRegime === 'WAIT_AND_SEE') {
          if (w.tickerReturn > 0) sizingSignals[w.sizingRegime].profitable++;
        } else {
          if (w.tickerReturn < 0) sizingSignals[w.sizingRegime].profitable++;
        }
      }
    ...
    res.json({
      ...
      watchlistSummary: {
        ...
        inconclusiveCalls: wlPerformance.filter((w: any) => w.modelCorrect === 'INCONCLUSIVE').length,
...
router.post('/lens/inject-bars', (req: Request, res: Response) => {
  ...
  try {
    setCachedBars(symbol.toUpperCase(), bars);
...
router.get('/lens/llm-test', async (_req: Request, res: Response) => {
  ...
  try {
    if (isAnthropic) {
      new Anthropic({ apiKey: envKey });
      clientOk = true;
    } else if (envKey) {
      new OpenAI({ apiKey: envKey, timeout: 5000 });
      clientOk = true;
    }
  } catch (err: any) {
