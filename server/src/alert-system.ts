...
const alertStore: InflectionAlert[] = [];
...
function isInCooldown(ticker: string, type: AlertType): boolean {
  const cooldownMs = COOLDOWN_DAYS * 24 * 60 * 60 * 1000;
  const now = Date.now();
  return alertStore.some(a =>
    a.ticker === ticker && a.type === type
    && now - new Date(a.timestamp).getTime() < cooldownMs
  );
}

function addAlert(alert: Omit<InflectionAlert, 'id' | 'timestamp' | 'expiresAt' | 'acknowledged'>): InflectionAlert | null {
  // Check cooldown
  if (isInCooldown(alert.ticker, alert.type)) return null;

  const now = new Date();
  const full: InflectionAlert = {
    ...alert,
    id: generateId(),
...
export function generatePhaseTransitionAlert(
  ticker: string,
  previousPhase: InflectionPhase | null,
  currentPhase: PhaseClassification,
  pillars: PillarScores,
): InflectionAlert | null {
  if (!previousPhase || previousPhase === currentPhase.phase) return null;

  const priority: AlertPriority =
    currentPhase.phase === 'SELLING_EXHAUSTION' || currentPhase.phase === 'BUYING_EXHAUSTION' ? 'HIGH'
...
export function generateExhaustionAlert(
  ticker: string,
  exhaustion: ExhaustionScore,
  pillars: PillarScores,
  phase: InflectionPhase,
): InflectionAlert | null {
  if (!exhaustion.triggered) return null;

  const type: AlertType = exhaustion.type === 'BUYING_EXHAUSTION' ? 'SHORT_NOW' : 'BUY_NOW';
...
export function generateDivergenceAlert(
  ticker: string,
  divergenceType: 'DECEL_DIVERGENCE' | 'ACCEL_DIVERGENCE',
  strength: number,
  price: number,
  pillars: PillarScores | null,
): InflectionAlert | null {
...
export function generatePillarExtremeAlert(
  ticker: string,
  pillars: PillarScores,
  phase: InflectionPhase,
): InflectionAlert | null {
  const extremes: string[] = [];
  if (pillars.technical > 85) extremes.push('Technical: EXTREMELY BULLISH');
  if (pillars.technical < 15) extremes.push('Technical: EXTREMELY BEARISH');
  if (pillars.fundamental > 85) extremes.push('Fundamental: EXTREMELY STRONG');
  if (pillars.fundamental < 15) extremes.push('Fundamental: EXTREMELY WEAK');
  if (pillars.valuation > 85) extremes.push('Valuation: EXTREMELY CHEAP');
  if (pillars.valuation < 15) extremes.push('Valuation: EXTREMELY EXPENSIVE');
  if (pillars.inflection > 85) extremes.push('Inflection: STRONG ACCELERATION');
  if (pillars.inflection < 15) extremes.push('Inflection: STRONG DECELERATION');
...
export function generateEarningsReactionAlert(
  ticker: string,
  reactionType: 'positive_surprise_flat' | 'negative_surprise_recovery',
  description: string,
  pillars: PillarScores | null,
): InflectionAlert | null {
...
export function generateIndustryShiftAlert(
  industry: string,
  ...
  if (activeSignals < totalDrivers * 0.5) return null; // Only alert if majority of drivers signaling
...
router.get('/', (req: Request, res: Response) => {
  ...
  if (ticker) alerts = alerts.filter(a => a.ticker === ticker.toUpperCase());
  ...
  if (req.query.unacked === 'true') alerts = alerts.filter(a => !a.acknowledged);
...
router.get('/critical', (_req: Request, res: Response) => {
  const now = Date.now();
  const critical = alertStore.filter(a =>
    a.priority === 'CRITICAL' && new Date(a.expiresAt).getTime() > now
  );
...
router.post('/acknowledge-all', (_req: Request, res: Response) => {
  let count = 0;
  alertStore.forEach(a => { if (!a.acknowledged) { a.acknowledged = true; count++; } });
