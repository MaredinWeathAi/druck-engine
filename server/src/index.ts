...
try {
  ...
  if (!anthropicApiKey) {
    const savedKey = getSetting('anthropic_api_key');
    if (savedKey) {
...
app.get('/api/health', (_req, res) => {
  ...
  res.json({
    status: 'ok',
    version: '16.1.0',
    build: '2026-06-29T22:00:00Z',
    BUILD_CANARY: 'EXPANDED_SEED_DATA_209_SYMBOLS',
...
app.get('/api/cycle-position', (_req, res) => {
  ...
  const liqVel13w = +(latV13 * 100).toFixed(2);
  let liqAccel = 0;
  if (vel4w.length >= 5) liqAccel = +((vel4w[vel4w.length-1] - vel4w[vel4w.length-5]) * 100).toFixed(3);
  ...
  const sectorPhases: Record<string, { phase: string; phaseNum: number; ext: number; verdict: string }> = {};
  const sectorEtfs = [
  ...
  if (currentPhase === 'EARLY_RECOVERY' || currentPhase === 'MID_CYCLE') {
    accumulate.push(
      { sector: 'Financials', etf: 'XLF', reason: 'Rate-sensitive cyclical, benefits from expansion', cycle_sweet_spot: 'Early-to-Mid' },
      { sector: 'Industrials', etf: 'XLI', reason: 'Capex cycle + ISM expansion', cycle_sweet_spot: 'Mid' },
      { sector: 'Technology', etf: 'XLK', reason: 'Earnings growth leads in expansion', cycle_sweet_spot: 'Early-to-Mid' },
      { sector: 'Materials', etf: 'XLB', reason: 'Commodity demand rises with activity', cycle_sweet_spot: 'Mid' },
    );
    hold.push(
      { sector: 'Healthcare', etf: 'XLV', reason: 'Defensive growth across cycles', cycle_sweet_spot: 'All' },
      { sector: 'Energy', etf: 'XLE', reason: 'Commodity support but watch late-cycle', cycle_sweet_spot: 'Mid-to-Late' },
    );
    reduce.push(
      { sector: 'Utilities', etf: 'XLU', reason: 'Underperforms in expansion', cycle_sweet_spot: 'Recession' },
      { sector: 'Staples', etf: 'XLP', reason: 'Defensive — rotate in when late signals emerge', cycle_sweet_spot: 'Late-to-Recession' },
    );
  } else if (currentPhase === 'LATE_CYCLE') {
    accumulate.push(
      { sector: 'Energy', etf: 'XLE', reason: 'Inflation tailwinds', cycle_sweet_spot: 'Late' },
      { sector: 'Healthcare', etf: 'XLV', reason: 'Defensive as growth slows', cycle_sweet_spot: 'All' },
      { sector: 'Staples', etf: 'XLP', reason: 'Rotate to defense', cycle_sweet_spot: 'Late-to-Recession' },
    );
    reduce.push(
      { sector: 'Technology', etf: 'XLK', reason: 'Valuation compression risk', cycle_sweet_spot: 'Early-to-Mid' },
      { sector: 'Cons Disc', etf: 'XLY', reason: 'Consumer weakening', cycle_sweet_spot: 'Early' },
    );
  } else if (currentPhase === 'RECESSION') {
    accumulate.push(
      { sector: 'Staples', etf: 'XLP', reason: 'Defensive earnings', cycle_sweet_spot: 'Recession' },
      { sector: 'Healthcare', etf: 'XLV', reason: 'Non-cyclical', cycle_sweet_spot: 'All' },
      { sector: 'Gold', etf: 'GLD', reason: 'Safe haven', cycle_sweet_spot: 'Recession' },
    );
    reduce.push(
      { sector: 'Technology', etf: 'XLK', reason: 'Earnings decline', cycle_sweet_spot: 'Early-to-Mid' },
      { sector: 'Financials', etf: 'XLF', reason: 'Credit losses', cycle_sweet_spot: 'Early-to-Mid' },
      { sector: 'Industrials', etf: 'XLI', reason: 'Capex cuts', cycle_sweet_spot: 'Mid' },
    );
  }
...
app.listen(PORT, () => {
  console.log(`\n  DRUCK ENGINE v16.1.0 — Structural Regime Intelligence + Expanded Seed Data (209 symbols)`);
